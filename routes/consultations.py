    ...
    def get(self):
        ...
        if department is not None:
            ...
            if department == 0 or department == 6:
                consultations = parse_consultations(links[department], True,
                                                    department_titles[department])
            elif 0 < department <= 5:
                consultations = parse_consultations(links[department], False,
                                                    department_titles[department])
            else:
                return None, 400
        else:
            for department, link in enumerate(links):
                if department == 0 or department == 6:
                    consultations += parse_consultations(link, True,
                                                         department_titles[department])
                else:
